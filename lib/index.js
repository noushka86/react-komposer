    ...
    const Container = class extends React.Component {
      ...
      render() {
        ...
        let loadingDisplay=0,
            errorDisplay=0,
            childrenDisplay=0;
        if (error) errorDisplay=1;
        else if (loading) loadingDisplay=1;
        else childrenDisplay = 1;
          return (
            <div>
              <div style={{ 'opacity': loadingDisplay }}><LoadingComponent /></div>
              <div style={{ 'opacity': errorDisplay }}><ErrorComponent error={error} /></div>
              <div style={{ 'display': childrenDisplay }}><ChildComponent {...this._getProps()} /></div>
            <div>
          );
        }
      }
